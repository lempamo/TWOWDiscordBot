...
module.exports = function(mainServer, chat, localData, idFromName, bot, clever, DEMIPIXEL_ID) {
	function message(user, userID, channelID, message, pm) {
		...
		if (cmd == 'create') {
			if (admins.indexOf(userID) == -1 && userID != DEMIPIXEL_ID && queue.length && userID != queue[0]) chat(channelID, '<@'+userID+'>: '+'You cannot create a game at this time. Try `!mini queue`.');
	...
	class Mini {
		...
		results() {
			...
			const displaySorted = sorted.map((player, index) => {
				var rank = index + 1;
				return '['+rank+'] <@'+player.id+'>: '+player.resp+' ('+player.gotVotes+' votes, '+Math.round(player.ratio*1000)/10+'%)'+(player.resp2 ? '\n    Response 2: '+player.resp2+' ('+player.gotVotes2+' votes, '+Math.round(player.ratio2*1000/10)+'%)':'');
			});
